...
class DummyPorts(Interface):
    def __init__(self, **kwargs):
        ...
        self.dummy_ports = {
             "left_port": "left",
             "right_port": "right",
             "proximity_true": "space",
             "proximity_false": "space"
         }
    ...
    def _get_events(self):
        port = 0
        keys = self.kb.getKeys(waitRelease=True)
        # events = pygame.event.get() if pygame.get_init() else []
        # events = psychopy.event.getKeys()
        for key in keys:
            print(f"Detected key: {event}")
            event = key.name
            prot = self._port_activated(event, port)
    ...
    def _port_activated(self, event, port):
        if self.dummy_ports_true(event, "left"):
            print("Probe 1 activated!")
            port = 1
        if self.dummy_ports_true(event, "right"):
    ...
    def dummy_ports_true(self, event, name):
        return event == self.dummy_ports[name]
        # if event == name:
