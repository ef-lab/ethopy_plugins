...
class Presenter:
    def __init__(
        ...
        self.win = psychopy.visual.Window(
            ...
            monitor="testMonitor",
            screen=monitor.screen_idx - 1,
            fullscr=monitor.fullscreen,
            useFBO=False,
            waitBlanking=True,
        )
        ...
        if monitor.flatness_correction:
            ...
            self.warper.changeProjection(
                "spherical", eyepoint=(monitor.center_x + 0.5, monitor.center_y + 0.5)
            )

        # record fliptimes for syncing
        self.rec_fliptimes = rec_fliptimes
        if self.rec_fliptimes:
            self.fliptimes_dataset = self.logger.createDataset(
                dataset_name="fliptimes",
                dataset_type=np.dtype([("flip_idx", np.double), ("tmst", np.double)]),
            )
        # initialize variables
        self.flip_count = 0
        self.photodiode = False  # photodiode
        self.phd_size = (
            0.1  # default photodiode signal size in ratio of the X screen size
        )
    ...
    def _setup_photodiode(self, photodiode=True):
        if photodiode:
            self.photodiode_rect = psychopy.visual.Rect(
                self.win,
                units="norm",
                width=self.phd_size,
            ...
            if photodiode == "parity":
                ...
                self.phd_f = lambda x: 0.5 * float(
                    ((x + 1) & 1)
                    * (
                        2
                        - (
                            (x + 1)
                            & (1 << (((np.int64(np.floor((x + 1) / 2)) & 15) + 6) - 1))
                            != 0
                        )
                    )
                )
                ...
                print(
                    photodiode,
                    " method not implemented! Available methods: parity, flipcount",
                )
    ...
    def flip(self):
        ...
        self.win.flip()
        if self.rec_fliptimes:
            self.fliptimes_dataset.append(
                "fliptimes", [self.flip_count, self.logger.logger_timer.elapsed_time()]
            )
        self.tick()
