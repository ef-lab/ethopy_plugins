...
class PsychoGrating(Stimulus, dj.Manual):
    ...
    def prepare(self, curr_cond, stim_period=""):
        ...
        for idx, pos_x in enumerate(iterable(self.curr_cond["pos_x"])):
            ...
            self.conds[idx] = cond
            self.gratings[idx] = psychopy.visual.GratingStim(
                self.Presenter.win,
    ...
    def present(self):
        ...
        for idx, grat in enumerate(iterable(self.curr_cond["pos_x"])):
            self.gratings[idx].phase += (
                self.conds[idx]["tf"] / self.monitor.fps
            )  # assuming 60 Hz refresh rate
    ...
    def _get_cond(self, idx=0):
        return {
            k: v
            if type(v) is int or type(v) is float or type(v) is str or type(v) is bool
            else v[idx]
