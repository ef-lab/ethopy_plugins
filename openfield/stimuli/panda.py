...
from ethopy.core.stimulus import (
    StimCondition,
    Stimulus,
)  # import StimCondition need for the Panda class definition
...
class Objects(dj.Lookup):
    ...
    def store(self, obj_id, file_name, description=""):
        tuple = dict(
            ...
            object=np.fromfile(file_name, dtype=np.int8),
            file_name=file_name,
        )
...
class Panda(Stimulus, dj.Manual):
    ...
    def init(self, exp):
        ...
        if not os.path.isdir(self.movie_path):
            os.mkdir(self.movie_path)
        if self.is_recording and not os.path.isdir(self.record_path):
    ...
    def setup(self):
        ...
        if self.monitor.fullscreen:
            self.props.setSize(
                self.pipe.getDisplayWidth(), self.pipe.getDisplayHeight()
            )
    ...
    def prepare(self, curr_cond, stim_period=""):
        ...
        for idx, obj in enumerate(iterable(self.curr_cond["obj_id"])):
            if self.curr_cond["perspective"]:
                self.objects[obj] = AgentPersp(self, self.get_cond("obj_", idx))
                print("self.curr_cond[perspective]", self.curr_cond["perspective"])
            else:
        ...
        if "movie_name" in self.curr_cond:
            ...
            self.movie_node.setTexScale(
                TextureStage.getDefault(), self.mov_texture.getTexScale()
            )
            self.movie_node.setScale(48)
            self.movie_node.reparentTo(self.render)

    def start(self):
        if self.flag_no_stim:
            return
        self.fill()
    ...
    def present(self):
        self.flip()
        if (
            "obj_dur" in self.curr_cond
            and self.curr_cond["obj_dur"] < self.timer.elapsed_time()
        ):
            self.in_operation = False
    ...
    def fill(self, color=None):
        if not color:
            color = self.curr_cond["background_color"]
    ...
    def start_recording(self):
        if self.is_recording:
            self.record_task = self.movie(
                namePrefix=self.record_path
                + "/trial"
                + str(self.exp.curr_trial)
                + "_frame",
                duration=self.curr_cond["obj_dur"],
                fps=self.fps,
                format="png",
            )
    ...
    def create_movies(self):
        ...
        for itrial in range(1, self.exp.curr_trial + 1):
            ...
            output_name = (
                self.record_path
                + str(self.exp.logger.trial_key["animal_id"])
                + "_"
                + str(self.exp.logger.trial_key["session"])
                + "_trial"
                + str(itrial)
                + ".mov"
            )
            ...
            os.system(
                ...
                + " -pattern_type glob -i "
                + f'"{name}"'
                + " -loglevel quiet -c:v libx264 -pix_fmt gray -crf 5 "
                + f'"{output_name}"'
            )
    ...
    def make_conditions(self, conditions):
        ...
        for cond in conditions:
            ...
            if not "obj_id" in cond:
                continue
            for obj_id in iterable(cond["obj_id"]):
                object_info = self.exp.logger.get(
    ...
    def get_clip_info(self, key, *fields):
        return self.logger.get(
            schema="stimulus", table="Movie.Clip", key=key, fields=fields
        )
...
class Agent(Panda):
    def __init__(self, env, cond):
        ...
        # self.env.camLens.set_fov((50.942, 30))
        ...
        if cond["occluder"]:
            self.z_loc = 1.5
        self.x_fun = self.time_fun(
            cond["pos_x"], lambda x, t: np.arctan(x * hfov[0]) * self.z_loc
        )
        self.y_fun = self.time_fun(
            cond["pos_y"], lambda x, t: np.arctan(x * hfov[0]) * self.z_loc
        )
        self.scale_fun = self.time_fun(cond["mag"], lambda x, t: 0.15 * x)
        # add task object
        self.name = "Obj%s-Task" % cond["id"]
        self.env.camera.setPos(0, 0, 0)
        # print("A#"*20)
        # print('self.env.camera.getPos() ', self.env.camera.getPos())
        self.first = True
    ...
    def run(self):
        ...
        self.task = self.env.taskMgr.doMethodLater(
            self.cond["delay"] / 1000, self.objTask, self.name
        )
    ...
    def objTask(self, task):
        ...
        if self.first:
            print("self.env.camera.getPos() ", self.env.camera.getPos())
            print("self.model.getPos() ", self.model.getPos())
            print("self.env.camLens.get_fov()  ", self.env.camLens.get_fov())
            self.first = False
        return Task.cont
...
class AgentPersp(Panda):
    def __init__(self, env, cond):
        ...
        hfov = self.env.camLens.get_fov() * np.pi / 180  # half field of view in radians
        # define object time parameters
        self.rot_fun = self.time_fun(0)
        self.tilt_fun = self.time_fun(cond["tilt"])
        self.yaw_fun = self.time_fun(0)
        self.z_loc = 2
        if cond["occluder"]:
            self.z_loc = 1.5
        self.x_fun = self.time_fun(
        ...
        self.subject_width = 30
        self.env.camera.setPos(0, -150, 0)

        # create a new numpy array that uses the shared memory
        self._dlc_pose = np.ndarray((1, 4), dtype=np.float32, buffer=self.env.sm.buf)
        self.tmst_cur, self.x_cur, self.y_cur, self.angle_cur = self._dlc_pose[0]

        self.x_cur = 0 if self.x_cur <= 0 else self.x_cur / 10
        self.x_cur = self.x_cur - 30
        # self.y_cur = 0 if self.y_cur<=0 else self.y_cur/10
        self.y_cur = (
            -np.abs(self.y_cur - 150) if self.y_cur < 150 else np.abs(self.y_cur - 150)
        )
        self.env.camera.setPos(0, self.y_cur, 0)
        self.camera_pos_x = self.x_cur
        self.first = True
        self.pos = self.env.camera.getPos()
        self.alpha = 0.5
        print("#" * 20)
        print("self.env.camera.getPos() ", self.env.camera.getPos())
    ...
    def objTask(self, task):
        ...
        if t > self.duration / 1000:
            ...
            return
        # if self.first:
        #     print(f"self.name {self.name} {self.model.getH()}")
        #     print("self.env.camera.getPos() ", self.env.camera.getPos())
        #     print("self.model.getPos() ", self.model.getPos())
        #     print("self.env.camLens.get_fov()  ", self.env.camLens.get_fov() )
        #     self.first = False
        # time.sleep(0.1)
        # # Move camera in Y direction with left/right arrows
        # if self.env.mouseWatcherNode.is_button_down(KeyboardButton.ascii_key('w')):
        #     self.env.camera.setY(self.env.camera, 1)
        # elif self.env.mouseWatcherNode.is_button_down(KeyboardButton.ascii_key('s')):
        #     self.env.camera.setY(self.env.camera, -1)

        # if self.env.mouseWatcherNode.is_button_down(KeyboardButton.ascii_key('d')):
        #     self.camera_pos_x += 0.5

        # elif self.env.mouseWatcherNode.is_button_down(KeyboardButton.ascii_key('a')):
        #     self.camera_pos_x -= 0.5

        self.tmst_cur, self.x_cur, self.y_cur, self.angle_cur = self._dlc_pose[0]

        # print("self.x_cur , self.y_cur ",self.x_cur , self.y_cur)
        self.x_cur = -(max(-30, min((-self.x_cur / 300) * 30, 0)) + 30)
        self.y_cur = max(-15, min((self.y_cur / 300) * 30 - 15, 15))
        # print("self.x_cur , self.y_cur ",self.x_cur , self.y_cur)

        # print(self.tmst_cur, self.x_cur, self.y_cur, self.angle_cur)
        # time.sleep(0.5)
        # self.x_cur = np.abs(self.x_cur/10-30)
        # self.y_cur = 0 if self.y_cur<0 else self.y_cur
        # self.x_cur = 0 if self.x_cur<0 else self.x_cur
        # print("self.y_cur "*30)
        # print(self.y_cur)
        # self.y_cur = -np.abs(self.y_cur-150) if self.y_cur<150 else np.abs(self.y_cur-150)
        # self.y_cur = self.y_cur/10
        self.env.camera.setY(self.x_cur)
        self.camera_pos_x = self.y_cur
        v = self.env.camera.getPos()
        v[0] = self.camera_pos_x
        u = self.model.getPos()
        # u[1] +=3 ################# check distance of angle

        # angle, dir = self.angle_between_vectors(u, v)
        # new_heading = (angle-180)*(-dir)
        angle = -(self.find_angle_3d(v, u) - 90)
        new_heading = -angle

        # print('self.camera.getPos() ', v)
        # print(" new_heading ", new_heading)
        if not np.isnan(angle):
            if (
                "prev_angle" not in locals()
            ):  # Initialize prev_angle if it doesn't exist
                prev_angle = angle
            filtered_angle = (
                self.alpha * prev_angle + (1 - self.alpha) * angle
            )  # Apply low-pass filter
            self.model.setH(filtered_angle)  # Use filtered_angle to set heading
            prev_angle = filtered_angle

        self.model.setP(self.tilt_fun(t))
        # self.camera.lookAt(self.model)
        self.dolly_zoom()

        # self.model.setPos(self.cond['pos_x']*25, 7, 0)
        # self.model.setScale(1.2, 1.2, 1.2)

        self.model.setPos(self.cond["pos_x"] * 30, 3, 0)
        self.model.setScale(1, 1, 1)

        return Task.cont

    def dolly_zoom(self):
        new_pos = self.env.camera.getPos()
        self.env.camera.setPos(new_pos)
        # Calculate the new FOV using the derived formula
        current_distance = abs(new_pos.getY() - self.model.getY())
        if current_distance > 1:
            new_fov = 2 * math.degrees(
                math.atan(self.subject_width / (2 * current_distance))
            )
            if self.pos != new_pos:
                self.pos = new_pos
            self.env.camLens.setFov(new_fov)

    def find_angle_3d(self, point1, point2):
        # Calculate the differences in coordinates
        delta = np.array(point2) - np.array(point1)
        # Project the vector onto the xy-plane (ignore the z-coordinate)
        delta_xy = delta[:2]
        return np.degrees(np.arctan2(delta_xy[1], delta_xy[0]))
