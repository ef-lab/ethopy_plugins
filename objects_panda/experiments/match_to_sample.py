...
class Cue(Experiment):
    ...
    def next(self):
        ...
        if self.resp_ready and (
            self.curr_cond["cue_ready"] or elapsed_time > self.curr_cond["cue_duration"]
        ):
            return "Delay"
        elif self.response:
            return "Abort"
...
class Response(Experiment):
    ...
    def next(self):
        if (
            self.response and self.beh.is_correct() and self.resp_ready
        ):  # correct response
            return "Reward"
            ...
            self.state_timer.elapsed_time() > self.curr_cond["response_duration"]
        ):  # timed out
            return "Abort"
        elif self.is_stopped():
            return "Exit"
